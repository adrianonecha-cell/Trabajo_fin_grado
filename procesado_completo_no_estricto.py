...
# -----------------------
# Alineamiento (par) - NO ESTRICTO
# -----------------------
def align_pair_no_strict(
    ref_bgr: np.ndarray,
    dst_bgr: np.ndarray,
    *,
    ...
    """
    Alinea dst -> ref usando directamente la matriz estimada por RANSAC (estimateAffinePartial2D).
    Esto puede incluir rotación + traslación + (posible) escala.
    """
    W, H = out_size
    ...
    # RANSAC: aquí SÍ usamos la matriz M_aff como transformación final
...
def align_sequence(
    ...
    *,
    ...
    for idx, p in enumerate(image_paths[1:], start=2):
        ...
        aligned, dbg = align_pair_no_strict(
            ref_current, dst,
            out_size=out_size,
...
def process_all(
    ...
    *,
    ...
    for gi, ((patient_id, marktext), g) in enumerate(grouped, start=1):
        ...
        # Si solo hay 1 imagen: guardar resize
        ...
        # Alinear secuencia completa (NO estricto)
        try:
            logs = align_sequence(
                image_paths,
                out_dir=out_folder,
...
def main():
    ...
    ap.add_argument("--out_root", type=str, default="./data_processed_no_strict", help="Directorio de salida (data_processed_no_strict).")
